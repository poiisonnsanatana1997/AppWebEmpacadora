...
export const ResumenProgreso: React.FC<ResumenProgresoProps> = ({ progreso }) => {
  ...
  // Calcular estadísticas básicas con memoización para optimizar rendimiento
  const estadisticas = useMemo(() => {
    ...
    return {
      totalCantidadRequerida,
      totalCantidadAsignada,
      totalCantidadFaltante,
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            {getProgressIcon(estadisticas.porcentajeCumplimiento)}
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{estadisticas.porcentajeCumplimiento}%</div>
            <Progress
              value={estadisticas.porcentajeCumplimiento}
              className="mt-2"
            />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{estadisticas.totalCantidadFaltante.toLocaleString()}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{estadisticas.totalCantidadAsignada.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              Ya asignadas
            </p>
          </CardContent>
